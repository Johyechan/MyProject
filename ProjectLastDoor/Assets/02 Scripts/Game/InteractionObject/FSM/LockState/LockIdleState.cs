...
namespace Game.InteractionObject.FSM
{
    ...
    public class LockIdleState : LockStateBase
    {
        ...
        public override void OnEnter()
        {
            ...
            _lock.gameObject.layer = LayerMask.NameToLayer(GameManager.Instance.InteractionLayerName); // 자물쇠(자기자신)를 상호작용 가능한 상태로 변경
            _lock.ChangeChannel(false); // 플레이어 시네머신으로 채널 전환
            _lock.IsFailed = false; // 상태 초기화 (실패 초기화)
            _lock.IsSuccess = false; // 상태 초기화 (최종적인 성공 여부 초기화)
            _lock.SuccessCount = 0; // 상태 초기화 (성공 초기화)
        }
    }
}
