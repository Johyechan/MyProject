...
namespace Game.Player
{
    ...
    public class PlayerMovement
    {
        ...
        {
            // 카메라의 x축만 회전하여 상하를 살필 수 있도록 함 + Space를 World로 하여 부모의 영향을 받지 않고 회전하도록 설정 + y축 앞에 '-'를 붙이는 이유는 마우스 좌표계와 유니티 좌표계가 반대이기 때문(예) 마우스를 위로 올리면 양수 값 양수 값이면 유니티에서는 아래로 회전함 그래서 -를 붙이면 마우스가 움직이는 방향에 맞게) 
